...
class FileAttribute
  ...
  def initialize(file_name)
    ...
    @block_num = (file_status.size / file_status.blksize.to_f).ceil * (file_status.blksize / UNIT_BLOCK_SIZE)
  end

  def output_file_infomation(maximum_link_digit, maximum_file_size_digit)
    [
      @mode.type + @mode.permission,
      @hard_link.to_s.rjust(maximum_link_digit),
      @user_name,
      @group_name,
